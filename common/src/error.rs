use crate::{openc2::OpenC2MsgType, PluginId};
...
pub enum Error {
    ...
    Serialize(RBoxError, OpenC2MsgType),
    /// An error produced by `serde_json::from_string`.
    Deserialize(RBoxError, OpenC2MsgType),
...
impl Error {
    ...
    pub fn invalid_plugin_id(id: PluginId) -> Self {
        Error::InvalidPlugin(id)
    }
}
        ...
        match self {
            Error::Serialize(e, which) => {
                let which = match which {
                    OpenC2MsgType::Request => "request",
                    OpenC2MsgType::Response => "response",
                };
                writeln!(
                    f,
            ...
            Error::InvalidPlugin(wc) => writeln!(
                f,
                "Attempted to access a nonexistent plugin :\n\t{:?}\n",
                wc
            ),
